...
function extractPriceFromTextHero(text) {
  ...
  const dealMatch = text.match(/(?:Deal\s+Price|With\s+deal|Price|Payable\s+Amount)\s*[:\s]*(?:₹|Rs\.?)\s*([\d,]+)/i);
  if (dealMatch) {
    const p = parsePrice(dealMatch[1]);
    if (p > 0) price = p;
  }

  const mrpMatch = text.match(/(?:M\.?R\.?P\.?|List\s+Price|Original\s+Price)\s*[:\s]*(?:₹|Rs\.?)\s*([\d,]+)/i);
  if (mrpMatch) {
  ...
  if (!price) {
    ...
    for (const rawLine of lines) {
      ...
      if (/emi|per\s+month|\/month|save\s+₹|upto|discount|cashback|off\s+on|exchange|protect|warranty|sponsored/i.test(line)) {
        continue;
      }
