import {
  fetchPage, extractBankOffers, parseStructuredBankOffers,
  parsePrice, decodeHTML, deepFind, isPlainText, sanitizeUrl,
  type ScrapedProduct,
} from './crawlerUtils';

async function fetchMyntraInternalApi(styleId: string | null, productUrl: string): Promise<ScrapedProduct | null> {
  ...
  for (const apiUrl of endpoints) {
    try {
      ...
      let image = '';
      if (style?.media?.albums?.[0]?.images?.[0]?.src) image = style.media.albums[0].images[0].src;
      if (!image) {
        const v = deepFind(json, ['imageURL', 'src', 'secureUrl']);
        if (v && typeof v === 'string') image = v;
      }

      if (price > 50 && title.length > 3) {
        return {
          url: productUrl, title: decodeHTML(title), price,
          originalPrice: style?.price?.mrp || price,
          rating: parseFloat(style?.ratings?.average) || 0,
          reviewCount: style?.ratings?.count || 0,
          availability: 'in_stock', sellerName: style?.seller?.name || '',
          image, rawOffers: [], bankOffers: [], domain: 'myntra', asin: null, lowestEver: 0,
        };
      }
    } catch {}
  }
...
function parseMyntraHTML(html: string, productUrl: string): ScrapedProduct {
  let title = '';
  ...
  if (!title) {
    ...
    if (metaTitle) title = decodeHTML(metaTitle[1].split('|')[0].replace('Buy', '').trim());
  }

  let price = 0;
  const priceMatch = html.match(/class=["'][^"']*pdp-price["'][^>]*>(?:<strong>)?(?:Rs\.?)?\s*([\d,]+)/i);
  if (priceMatch) { const p = parsePrice(priceMatch[1]); if (p > 50) price = p; }
  ...
  return {
    ...
    domain: 'myntra', asin: null, lowestEver: 0,
  };
}
...
export async function scrapeMyntra(productUrlInput: string): Promise<ScrapedProduct> {
  const productUrl = sanitizeUrl(productUrlInput);
  const styleId = productUrl.match(/\/(\d+)\/buy/i)?.[1] ||
                  productUrl.match(/[^\d](\d{6,10})(?:[^\d]|$)/)?.[1] || null;

  const apiResult = await fetchMyntraInternalApi(styleId, productUrl);
  if (apiResult) return apiResult;

  const html = await fetchPage(productUrl, 'myntra');
  return isPlainText(html) ? parseFromText(html, productUrl) : parseMyntraHTML(html, productUrl);
}
